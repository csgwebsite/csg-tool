...
const SUPABASE_URL = 'https://oglftbigzlklvozygvrt.supabase.co';
const SUPABASE_ANON_KEY = 'sb_publishable_TUEKjCIg0Sn6Fr1l7Are3Q_mLJXzfKn';
...
CSG15106,Trần Trà Giang,SS180038,934994910,,,giangttss180038@fpt.edu.vn,CSG - HCM CAMPUS,,,K18,66304002496,,16/05/2004,8/12/2023,FALSE,,,,,,,,https://www.facebook.com/giahan280105?mibextid=ZbWKwL,,,,,,
...
for (let i = 1; i < lines.length; i++) {
    ...
    let accessHistory = [];
    if (obj.accessHistory) {
        try { accessHistory = JSON.parse(obj.accessHistory); } catch(e) {}
    }
    let tags = [];
    if (obj.tags) {
    ...
    let projectRoles = {};
    if (obj.projectRoles) {
        try { projectRoles = JSON.parse(obj.projectRoles); } catch(e) {}
    }
    ...
    rows.push({
        ...
        mssv: obj.mssv || null,
        phone: formatPhone(obj.phone),
        emailFE: obj.emailFE || null,
        emailFPT: obj.emailFPT || null,
        gmail: obj.gmail || null,
        location: obj.location || null,
        position: obj.position || null,
        school: obj.school || null,
        generation: obj.generation || null,
        cccd: formatCccd(obj.cccd),
        mst: obj.mst || null,
        dob: parseDate(obj.dob),
        startDate: parseDate(obj.startDate),
        isAdmin: obj.isAdmin?.toUpperCase() === 'TRUE',
        isMaster: obj.isMaster?.toUpperCase() === 'TRUE',
        ...
        tiktok: obj.tiktok || null,
        bankName: obj.bankName || null,
        bankAccount: obj.bankAccount || null,
        bankAccountName: obj.bankAccountName || null,
        bankBranch: obj.bankBranch || null,
        status: obj.status || 'active'
    });
}

console.log(`Parsed ${rows.length} members with updated Facebook links. Uploading to Supabase...`);
...
async function uploadToSupabase() {
    ...
    for (let i = 0; i < rows.length; i += chunkSize) {
        ...
        if (error) {
            console.error(`Error uploading batch ${Math.floor(i / chunkSize) + 1}:`, error);
            process.exit(1);
        } else {
            console.log(`Uploaded batch ${Math.floor(i / chunkSize) + 1} (${chunk.length} members)`);
        }
    }
    
    // Check count of members with facebook link
    const { data: fbMembers, error: fbErr } = await supabase
        .from('members')
        .select('id, fullName, facebook')
        .not('facebook', 'is', null);
    
    if (fbErr) {
        console.error('FB count error:', fbErr);
    } else {
        console.log(`🎉 SUCCESS! Total members with Facebook link now: ${fbMembers.length}`);
    }
}
